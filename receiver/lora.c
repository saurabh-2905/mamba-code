...
int receive(char *payload, int * receivedbytes) {
...
void receivepacket(char * buffer, int * receivedbytes, int * prssi, int * rssi, long int *snr, int *error) {
  int rssicorr;
  int ret;
  *error = 0;
  ...
  if(digitalRead(dio0) == 1)
    {
      ret = receive(buffer, receivedbytes);
      if(ret == 0){
...
static PyObject* recv(PyObject* self, PyObject* args) {
  ...
  receivepacket(buffer, &receivedbytes, &prssi, &rssi, &snr, &error);

  return Py_BuildValue("y#iiiii", buffer, receivedbytes, receivedbytes, prssi, rssi, snr, error);
}


static PyObject* changemode(PyObject* self, PyObject* args) {
  int mode;
...
static PyObject* init(PyObject* self, PyObject* args) {
  ...
  if (!PyArg_ParseTuple(args, "ii", &freq, &sf)){
    return NULL;
  }

  if((sf < 7) || (sf > 12)){
      printf("Bad spreading factor. Should be 7, 8, 9, 10, 11 or 12\r\n");
...
static PyMethodDef LoraMethods[] = {
	...
	{"changemode",  changemode, METH_VARARGS, "Change mode"},
